export class StatsSerializer {
  static getSeasonsStats(driverStats) {
    const statsBySeason = driverStats.MRData.RaceTable.Races.reduce(
      (aggObj, curObj) => {
        ...
        if (curObj.Results[0].grid === "1") {
          aggObj[curObj["season"]].totalPoles =
            aggObj[curObj["season"]].totalPoles + 1
        }
        // total podiums
        if (
          ...
          aggObj[curObj["season"]].totalPodiums =
            aggObj[curObj["season"]].totalPodiums + 1
        }
        ...
        aggObj[curObj["season"]].avgRaceFinish =
          aggObj[curObj["season"]].avgRaceFinish +
          parseInt(curObj.Results[0].position)
        ...
        aggObj[curObj["season"]].avgPointsPerRace =
          aggObj[curObj["season"]].avgPointsPerRace +
          parseInt(curObj.Results[0].points)
    ...
    for (const season in statsBySeason) {
      ...
      // average speed - final
      if (statsBySeason[season].avgSpeed === 0) {
        statsBySeason[season].avgSpeed = "N/A"
      } else {
